...
func (s *session) close() {
	s.once.Do(func() {
		if s.tcp != nil { _ = s.tcp.Close() }
		if s.ws != nil { _ = s.ws.Close() }
	})
}
...
func usage() {
	fmt.Println(`Usage: ghost <server|client> [options]
...
func parse(args []string) (config, error) {
	...
	ssh := fs.Int("ssh", 22, "ssh port")
	connect := fs.String("connect", "", "remote URL")
	quiet := fs.Bool("quiet", false, "quiet")
	verbose := fs.Bool("verbose", false, "verbose")
	v := fs.Bool("v", false, "info")
	vv := fs.Bool("vv", false, "debug")
	vvv := fs.Bool("vvv", false, "verbose")
	...
	if c.mode == "server" {
		c.sshPort = *ssh
		if *port != 0 { c.httpPort = *port }
	} else if c.mode == "client" {
		if *port != 0 { c.localPort = *port }
		if c.remoteURL == "" { return c, fmt.Errorf("--connect is required in client mode") }
	} else { return c, fmt.Errorf("unknown mode %q", c.mode) }
	return c, nil
}
...
func wsURL(raw string) (string, error) {
	...
	if u.Host == "" { return "", fmt.Errorf("URL has no host") }
	u.Scheme = "wss"
...
func serveWS(w http.ResponseWriter, r *http.Request, c config) {
	...
	if err != nil { debugf("websocket upgrade: %v", err); return }
	defer ws.Close()

	tcp, err := net.Dial("tcp", fmt.Sprintf("127.0.0.1:%d", c.sshPort))
	if err != nil { debugf("connect sshd: %v", err); return }
	defer tcp.Close()
	infof("accepted tunnel")
	bridge(&session{tcp: tcp, ws: ws})
}

func runClient(c config) error {
	remote, err := wsURL(c.remoteURL)
	if err != nil { return err }
	ln, err := net.Listen("tcp", fmt.Sprintf("127.0.0.1:%d", c.localPort))
	if err != nil { return err }
	defer ln.Close()
	infof("GhostSSH client listening on localhost:%d", c.localPort)
	infof("remote websocket: %s", remote)
	stop := make(chan os.Signal, 1)
	signal.Notify(stop, os.Interrupt, syscall.SIGTERM)
	...
	for {
		tcp, err := ln.Accept()
		if err != nil {
			if errorsClosed(err) { return nil }
			return err
		}
		go clientSession(tcp, remote)
	}
}
...
func clientSession(tcp net.Conn, remote string) {
	...
	ws, _, err := d.Dial(remote, nil)
	if err != nil { debugf("websocket dial: %v", err); return }
	defer ws.Close()
	infof("client tunnel established")
	bridge(&session{tcp: tcp, ws: ws})
}
...
func bridge(s *session) {
	...
	go func() {
		...
		for {
			...
			if n > 0 {
				debugf("TCP -> WS: %d bytes", n)
				if err := s.ws.WriteMessage(websocket.BinaryMessage, buf[:n]); err != nil { return }
				verbosef("forwarded TCP -> WS: %d bytes", n)
			}
			if err != nil { return }
		}
	}()
	go func() {
		defer wg.Done()
		for {
			t, data, err := s.ws.ReadMessage()
			if err != nil { return }
			if t != websocket.BinaryMessage && t != websocket.TextMessage { continue }
			debugf("WS -> TCP: %d bytes", len(data))
			if _, err := s.tcp.Write(data); err != nil { return }
			verbosef("forwarded WS -> TCP: %d bytes", len(data))
		}
	}()
	wg.Wait()
	s.close()
...
func errorsClosed(err error) bool {
	if err == net.ErrClosed { return true }
	return strings.Contains(strings.ToLower(err.Error()), "use of closed network connection")
}

func main() {
	c, err := parse(os.Args[1:])
	if err != nil { usage(); logger.Printf("error: %v", err); os.Exit(1) }
	if c.mode == "server" { err = runServer(c) } else { err = runClient(c) }
	if err != nil { logger.Printf("error: %v", err); os.Exit(1) }
}
